...
//
// Docked to the right so the structure + active lens stay VISIBLE on the left:
// each lens step calls onFocusLens(concept), which selects the example protein
// that best demonstrates that lens and switches the overlay on, so the viewer
// lights up behind the tour. Pure UI over grounded data (conceptDefs +
// paperGrounding). The Evoformer step renders the teaching schematic.
...
function buildSteps(defs, glossary, equationDeck) {
  ...
  return [
    ...
    { tag: "INPUT", focus: "coevolution", color: defs.coevolution.color, title: "Read evolution: sequence → MSA → coevolution",
...
export default function TourOverlay({ open, onClose, conceptDefs, glossary, equationDeck, colors, onFocusLens }) {
  ...
  return h("div", { "data-testid": "tour-overlay", style: { position: "fixed", top: 0, right: 0, height: "100vh", width: "min(384px,94vw)", zIndex: 50, background: "linear-gradient(180deg,#150f30,#0c0820)", borderLeft: `1px solid ${s.color}`, boxShadow: `-18px 0 60px rgba(0,0,0,.5)`, overflowY: "auto", padding: "18px 20px" } },
    h("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 } },
      ...
      h("button", { onClick: onClose, "aria-label": "Close tour", style: { background: "none", border: "none", color: "#9d8fd6", fontSize: 18, cursor: "pointer", lineHeight: 1 } }, "✕")),
    s.focus ? h("div", { style: { fontFamily: mono, fontSize: 8.5, color: s.color, marginBottom: 8, opacity: 0.9 } }, "◉ lens lit on the structure behind →") : null,
    h("div", { style: { fontFamily: mono, fontWeight: 800, fontSize: 16.5, color: "#f3f0ff", lineHeight: 1.25, marginBottom: 9 } }, s.title),
    h("div", { style: { fontSize: 12.5, lineHeight: 1.55, color: "#cabbf0" } }, s.body),
    s.render === "evoformer" ? h("div", { style: { marginTop: 12, borderRadius: 12, border: "1px solid #2c2350", background: "#0a0716", padding: 8 } }, h(EvoformerDiagram, { colors: { coev: C.coev, tri: C.tri, ipa: C.ipa, fape: C.fape, rec: C.rec } })) : null,
    s.why ? h("div", null, label("THE QUESTION", s.color), h("div", { style: { fontSize: 12, lineHeight: 1.5, color: "#e8e3ff", fontStyle: "italic" } }, s.why)) : null,
    s.boundary ? h("div", null, label("HONEST BOUNDARY", "#ffb347"), h("div", { style: { fontSize: 11, lineHeight: 1.5, color: "#d9b98a" } }, s.boundary)) : null,
    s.cite ? h("div", null, label("PAPER", "#7a6aa8"), h("div", { style: { fontSize: 10.5, color: "#9d8fd6", fontFamily: mono } }, s.cite)) : null,
    h("div", { style: { display: "flex", gap: 5, justifyContent: "center", margin: "16px 0 12px", flexWrap: "wrap" } },
      steps.map((_, k) => h("button", { key: k, "aria-label": `Go to step ${k + 1}`, onClick: () => setI(k), style: { width: k === idx ? 20 : 8, height: 8, borderRadius: 4, border: "none", cursor: "pointer", background: k === idx ? s.color : "#3a2f63", transition: "all .2s" } }))),
    h("div", { style: { display: "flex", justifyContent: "space-between", gap: 10 } },
      h("button", { onClick: () => setI(Math.max(0, idx - 1)), disabled: idx === 0, style: { flex: 1, height: 38, borderRadius: 10, border: "1px solid #4a3d72", background: idx === 0 ? "#0a0612" : "#1d1640", color: idx === 0 ? "#4a3d72" : "#cabbf0", fontFamily: mono, fontWeight: 700, fontSize: 11.5, cursor: idx === 0 ? "default" : "pointer" } }, "◀ BACK"),
      idx === steps.length - 1
        ...
        : h("button", { onClick: () => setI(Math.min(steps.length - 1, idx + 1)), style: { flex: 1, height: 38, borderRadius: 10, border: "none", background: `linear-gradient(135deg,${s.color},#2fd6ff)`, color: "#08060f", fontFamily: mono, fontWeight: 800, fontSize: 11.5, cursor: "pointer" } }, "NEXT ▶")));
}
